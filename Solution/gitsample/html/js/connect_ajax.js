...
function buildFormUrlParameters(ns, validateForm, inline, sendOnlyValid, includeFileInput)
{
	...
	for (var i = 0; i < idArr.length; i++)
	{
		...
		if (name != null && name.length > 0 && el.nodeName != "FIELDSET")
		{
			...
			//keep the selected files to add them after the ajax call
			if (el.type == "file"){
				setVariable(ns, "FILE_FILES"+el.id, el.files);
			}
		...
		try{
			try {
				...
				if (ajaxCaller != "TetrisUpdate") {
					...
					runEvalForLD(window, actionData);// used for LD
				}
			} catch (e) {
				try {
...
function runEvalForLD(windowObj, actionData) {
	return true;
}
...
function postAjaxFocusOn(ns, focusId)
{
    ...
    //if the field that triggered the ajax call is a file, the selected files should be restored
    if ( field != null && field.type == "file"){
        var files=getVariable(ns, "FILE_FILES"+focusId);
        if (files) {
            field.files = files;
        }
    }
