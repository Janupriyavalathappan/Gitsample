 ...
 if (!this["beforeValidation"] || this["beforeValidation"](comp, getMsg(ns, "InvalidUploadTypeMessage", "QUESTION_TEXT=" + getLabelText(ns, comp)), null, null, min, max, message, validFileExts, null, ns, null, null, styles) ){
  if (!this["beforeValidFile"] || beforeValidFile(comp, validFileExts, message, ns) ){
   ...
   if (!ok){
    ...
    if (ns == null) ns = "";
    var invalidQuestions = getVariable(ns, "invalidQuestions") || [];
    if(anyErrors){  
     if (message[0]!= null && message[0].length > 0) alertComp(comp,message[0],ns, inline);
     else alertComp(comp,errorMessage, ns, inline);
     //add question to list of invalid questions
     invalidQuestions[comp.id] = comp;
     setVariable(ns, "invalidQuestions", invalidQuestions);
     comp.setAttribute("aria-invalid", "true");
    }
    else
    {
     //remove question from invalid questions
     delete invalidQuestions[comp.id];
     comp.removeAttribute("aria-invalid");
     }
    ok = !anyErrors;
   }
  }
